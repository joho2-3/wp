...
foreach( (array)$payments as $id => $payment ) {
	if( 'acting_paypal_ec' == $payment['settlement'] && 'activate' == $payment['use'] ) { $usces_payment = true; break; }
}
if( $usces_payment ) {
...
function usces_paypal_filter_cart_page_footer( $html ) {
	global $usces;
	$footer = ( usces_is_cart_page() ) ? usces_paypal_cart_page_footer( false ) : '';
	return $html.$footer;
}
...
function usces_paypal_footer_scripts() {
	...
	$usces_entries = $usces->cart->get_entry();
	$total_price = $usces_entries['order']['total_items_price'] + $usces_entries['order']['discount'] + $usces_entries['order']['shipping_charge'] + $usces_entries['order']['cod_fee'] + $usces_entries['order']['tax'];
...
jQuery(function($) {
	paypalfunc = {
		...
		usePoint: function() {
			var s = this.settings;
			s.data = "usces_ajax_action=paypal_use_point&usepoint=" + $("#set_usedpoint").val() + "&total_price=<?php echo $total_price; ?>&item_price=<?php echo $item_price; ?>&tax=<?php echo $tax; ?>";
			s.success = function( res, dataType ) {
				var r = res.split( "#usces#" );
				if( r[0] == "error" ) {
...
function usces_paypal_front_ajax() {
	...
	case 'paypal_use_point':
		usces_paypal_use_point( $_POST['usepoint'], $_POST['total_price'], $_POST['item_price'], $_POST['tax'] );
		break;
...
function usces_paypal_use_point( $usepoint, $total_price, $item_price, $tax ) {
	global $usces;
...
//			$mes = __('You have exceeded the maximum available.', 'usces').' '.__('Max','usces').' '.$total_price.' '.__('points','usces');
