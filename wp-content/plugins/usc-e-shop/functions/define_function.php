...
//	define('IDENTIFIER_OLE', pack("CCCCCCCC",0xd0,0xcf,0x11,0xe0,0xa1,0xb1,0x1a,0xe1));
	...
	$comp_num = 0;
	$err_num = 0;
	$min_field_num = apply_filters( 'usces_filter_uploadcsv_min_field_num', 35 + $add_field_num );
	$log = '';
	$pre_code = '';
	$sku_index = 0;
	$res = array();
...
//usces_log('mysql2date : '.mysql2date('U', $cdatas['post_modified'], false) . '/' . mysql2date('U', current_time( 'mysql' ), false), 'itemcsv_log.txt');
			...
			if($mode == 'add') {
				...
				if( $dbres === false ) {
					$err_num++;
					$mes = "No." . ($rows_num+1) . "\t".__('Error : delete wcct', 'usces');
					$log .= $mes.$yn;
					echo $mes.$br.$yn;
					$pre_code = $item_code;
					continue;
...
//			wp_set_object_terms($post_id, (array)$tags, 'post_tag');
			...
			do_action( 'usces_action_uploadcsv_itemvalue', $post_id, $datas );
...
//		usces_log('skuvalue : '.$post_id.print_r($skuvalue,true), 'itemcsv_log.txt');
		$comp_num++;
		$pre_code = $item_code;
