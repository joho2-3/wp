...
function usces_tax( $data, $out = '' ){
	...
	if( !usces_is_tax_display() ) {
		...
		if( 'exclude' == $usces->options['tax_mode'] ){
			if( array_key_exists( 'order', $data ) && array_key_exists( 'tax', $data['order'] ) ) {
				$tax = $data['order']['tax'];
			} elseif( array_key_exists( 'order_tax', $data ) ) {
				$tax = $data['order_tax'];
...
function usces_internal_tax( $materials, $out = '' ){
	...
	if( !usces_is_tax_display() ) {
		...
		if( 1 == $usces->options['point_coverage'] ) {
...
function usces_direct_intoCart($post_id, $sku, $force=false, $value=NULL, $options=NULL, $out = '') {
	...
	$usces->itemopts = usces_get_opts($post_id, 'sort');
...
//				<table id="retail_table_' . $umhs['ID'] . '" class="retail">';
	...
	//		$cartItemName = $this->getCartItemName($post_id, $sku_code);
			...
			//$skuPrice = $usces->getItemPrice($post_id, $sku_code);
			...
			$cart_item_name = apply_filters( 'usces_filter_history_cart_item_name', $cart_item_name, $cartItemName, $optstr, $cart_row, $i );
...
function uesces_addressform( $type, $data, $out = 'return' ){
	...
	if( 'confirm' == $type ){
		...
		case 'JP':
			...
			$formtag .= '<tr><th>'.apply_filters( 'usces_filters_addressform_name_label', __('Full name', 'usces'), $type, $values, $applyform ).'</th><td>' . sprintf(_x('%s', 'honorific', 'usces'), (esc_html($values['customer']['name1']) . ' ' . esc_html($values['customer']['name2'])) ) . '</td></tr>';
			...
			$furigana_customer = '<tr><th>'.__('furigana', 'usces').'</th><td>' . $furigana . '</td></tr>';
...
//20131213_kitamura_end
			...
			$formtag .= '
			<tr><th>'.__('Province', 'usces').'</th><td>' . esc_html($customer_pref) . '</td></tr>
			...
			if( isset($values['delivery']) ) {
				...
				$furigana_delivery = '<tr><th>'.__('furigana', 'usces').'</th><td>' . $deli_furigana . '</td></tr>';
	...
	//20131213_kitamura_end
				...
				$shipping_address_info .= '
				<tr><th>'.__('Province', 'usces').'</th><td>' . esc_html($delivery_pref) . '</td></tr>
		...
		case 'CN':
			$formtag .= usces_custom_field_info($data, 'customer', 'name_pre', 'return');
			$formtag .= '<tr><th>'.apply_filters( 'usces_filters_addressform_name_label', __('Full name', 'usces'), $type, $values, $applyform ).'</th><td>' . sprintf(_x('%s', 'honorific', 'usces'), esc_html(usces_localized_name( $values['customer']['name1'], $values['customer']['name2'], 'return' )) ) . '</td></tr>';
			$formtag .= usces_custom_field_info($data, 'customer', 'name_after', 'return');
//20131213_kitamura_start
			if( count( $options['system']['target_market'] ) != 1 ){
				$customer_country = (!empty($usces_settings['country'][$values['customer']['country']])) ? $usces_settings['country'][$values['customer']['country']] : '';
...
//20131213_kitamura_end
			$customer_pref = ( $values['customer']['pref'] == __('-- Select --','usces') || $values['customer']['pref'] == '-- Select --' ) ? '' : $values['customer']['pref'];
			$formtag .= '
			<tr><th>'.__('State', 'usces').'</th><td>' . esc_html($customer_pref) . '</td></tr>
			<tr><th>'.__('city', 'usces').'</th><td>' . esc_html($values['customer']['address1']) . '</td></tr>
			...
			if( isset($values['delivery']) ) {
				$shipping_address_info = '<tr class="ttl"><td colspan="2"><h3>'.__('Shipping address information', 'usces').'</h3></td></tr>';
				$shipping_address_info .= usces_custom_field_info($data, 'delivery', 'name_pre', 'return');
				$shipping_address_info .= '<tr><th>'.apply_filters( 'usces_filters_addressform_name_label', __('Full name', 'usces'), $type, $values, $applyform ).'</th><td>' . sprintf(_x('%s', 'honorific', 'usces'), esc_html(usces_localized_name( $values['delivery']['name1'], $values['delivery']['name2'], 'return' )) ) . '</td></tr>';
	...
	//20131213_kitamura_end
				$delivery_pref = ( $values['delivery']['pref'] == __('-- Select --','usces') || $values['delivery']['pref'] == '-- Select --' ) ? '' : $values['delivery']['pref'];
				$shipping_address_info .= '
				<tr><th>'.__('State', 'usces').'</th><td>' . esc_html($delivery_pref) . '</td></tr>
				<tr><th>'.__('city', 'usces').'</th><td>' . esc_html($values['delivery']['address1']) . '</td></tr>
		...
		default :
			$customer_pref = ( $values['customer']['pref'] == __('-- Select --','usces') || $values['customer']['pref'] == '-- Select --' ) ? '' : $values['customer']['pref'];
			$formtag .= usces_custom_field_info($data, 'customer', 'name_pre', 'return');
			$formtag .= '<tr><th>'.apply_filters( 'usces_filters_addressform_name_label', __('Full name', 'usces'), $type, $values, $applyform ).'</th><td>' . sprintf(_x('%s', 'honorific', 'usces'), (esc_html($values['customer']['name2']) . ' ' . esc_html($values['customer']['name1'])) ) . '</td></tr>';
			$formtag .= usces_custom_field_info($data, 'customer', 'name_after', 'return');
			$formtag .= '
			<tr><th>'.__('Address Line1', 'usces').'</th><td>' . esc_html($values['customer']['address2']) . '</td></tr>
			<tr><th>'.__('Address Line2', 'usces').'</th><td>' . esc_html($values['customer']['address3']) . '</td></tr>
...
//20131213_kitamura_end
			...
			if( isset($values['delivery']) ) {
				...
				$shipping_address_info = '<tr class="ttl"><td colspan="2"><h3>'.__('Shipping address information', 'usces').'</h3></td></tr>';
				$shipping_address_info .= usces_custom_field_info($data, 'delivery', 'name_pre', 'return');
				$shipping_address_info .= '<tr><th>'.apply_filters( 'usces_filters_addressform_name_label', __('Full name', 'usces'), $type, $values, $applyform ).'</th><td>' . sprintf(_x('%s', 'honorific', 'usces'), (esc_html($values['delivery']['name2']) . ' ' . esc_html($values['delivery']['name1'])) ) . '</td></tr>';
				$shipping_address_info .= usces_custom_field_info($data, 'delivery', 'name_after', 'return');
				$shipping_address_info .= '
				<tr><th>'.__('Address Line1', 'usces').'</th><td>' . esc_html($values['delivery']['address2']) . '</td></tr>
				<tr><th>'.__('Address Line2', 'usces').'</th><td>' . esc_html($values['delivery']['address3']) . '</td></tr>
...
function usces_postal_code_address_search( $type, $out = 'return' ) {
	...
	if( isset($usces->options['address_search']) && $usces->options['address_search'] == 'activate' ) {
		$html = '<input type="button" id="search_zipcode" class="search-zipcode button" value="住所検索" onClick="AjaxZip3.zip2addr(\''.esc_attr($type.'[zipcode]').'\', \'\', \''.esc_attr($type.'[pref]').'\', \''.esc_attr($type.'[address1]').'\');" >';
	}
	if( $out == 'return' ) {
		return $html;
	} else {
