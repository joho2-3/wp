...
//			$optstr = implode(',', $options);
	...
	if( usces_is_member_system() && usces_is_member_system_point() && 0 == usces_point_coverage() && $data['order_usedpoint'] != 0 )
	...
	if( usces_is_member_system() && usces_is_member_system_point() && 1 == usces_point_coverage() && $data['order_usedpoint'] != 0 )
		$meisai .= __('use of points','usces') . " : " . number_format($data['order_usedpoint']) . __('Points','usces') . "\r\n";
	...
	if( $_POST['mode'] == 'completionMail' ){
		...
		if( !empty($delivery_company) ){
			...
			if( !empty($tracking_number) ){
				...
				$msg_shipping .= "■" . __('You can check delivery situation from the following URL.', 'usces') . "\r\n";
...
//			$optstr = implode(',', $options);
	...
	$order_para = array(
			...
			'to_address' => $usces->options['order_mail'], 
//			'from_name' => sprintf(_x('%s', 'honorific', 'usces'), ($entry["customer"]["name1"] . ' ' . $entry["customer"]["name2"])),
...
function usces_send_inquirymail() {
	...
	if ( $res0 ) {
		...
		$para2 = array(
				...
				//'from_name' => sprintf(__('Dear %s', 'usces'), $inq_name),
...
//
//	$para1 = array(
//			'to_name' => sprintf(_x('%s', 'honorific', 'usces'), $_SESSION["usces_lostmail"]),
...
//	$wpdb->show_errors();
	...
	if ( !$order_id ) :
		...
		if( isset($_REQUEST['res_tracking_id']) ) {
			...
			$usces->set_order_meta_value( 'wc_trans_id', $_REQUEST['res_tracking_id'], $order_id );
		}
		//*** paydesign (digitalcheck) ***//
		...
		//*** zeus card ***//
		if( $set['settlement'] == 'acting_zeus_card' and !empty($results['order_number']) ) {
			$usces->set_order_meta_value( 'wc_trans_id', $results['order_number'], $order_id );
		//*** zeus conv ***//
		} elseif( $set['settlement'] == 'acting_zeus_conv' and !empty($results['wctid']) ) {
		...
		if( isset($_REQUEST['acting']) && 'telecom_card' == $_REQUEST['acting'] && isset($_REQUEST['acting_return']) && isset($_REQUEST['option']) ) {
			...
			if( 'on' == $acting_opts['oneclick'] ) {
				$usces->set_member_meta_value( 'telecom_oneclick', $member['ID'] );
			}
		}
...
//$wpdb->show_errors();
	...
	if( isset($_POST['telecom_oneclick']) and $_POST['telecom_oneclick'] == 'delete' ) {
		$res[$i] = $usces->del_member_meta( 'telecom_oneclick', $ID );
	}
...
//	$item_total_price = $usces->get_total_price_ordercart( $new_cart );
	//$set = $usces->getPayments( $entry['order']['payment_name'] );
...
function usces_paypal_doecp( &$results ) {
	...
	if( 'continue' != $charging_type ) {
		...
		foreach( $cart as $cart_row ) {
			$nvpstr .= 
				'&L_PAYMENTREQUEST_0_NAME'.$i.'='.$usces->getItemName($cart_row['post_id']).
				'&L_PAYMENTREQUEST_0_AMT'.$i.'='.usces_crform($cart_row['price'], false, false, 'return', false).
				'&L_PAYMENTREQUEST_0_NUMBER'.$i.'='.$usces->getItemCode($cart_row['post_id']).
			...
			if( is_array($options) && count($options) > 0 ) {
				...
				if( $optstr != '' ) {
					if( 60 < mb_strlen($optstr) ) $optstr = mb_substr($optstr, 0, 60).'...';
					$nvpstr .= '&L_PAYMENTREQUEST_0_DESC'.$i.'='.$optstr;
				}
			}
		...
		if( !empty($entry['order']['discount']) ) {
			$nvpstr .= 
				'&L_PAYMENTREQUEST_0_NAME'.$i.'='.__('Campaign disnount', 'usces').
		...
		if( !empty($entry['order']['usedpoint']) ) {
			$nvpstr .= 
				'&L_PAYMENTREQUEST_0_NAME'.$i.'='.__('Used points', 'usces').
		...
		if( 'shipped' == $usces->getItemDivision( $post_id ) ) {
			...
			if( 'US' != $country and 'CA' != $country ) {
				$nvpstr .= 
				'&PAYMENTREQUEST_0_SHIPTONAME='.$entry['delivery']['name2'].' '.$entry['delivery']['name1'].
				'&PAYMENTREQUEST_0_SHIPTOSTREET='.$entry['delivery']['address2'].
				'&PAYMENTREQUEST_0_SHIPTOSTREET2='.$entry['delivery']['address3'].
				'&PAYMENTREQUEST_0_SHIPTOCITY='.$entry['delivery']['address1'].
				'&PAYMENTREQUEST_0_SHIPTOSTATE='.$entry['delivery']['pref'].
				'&PAYMENTREQUEST_0_SHIPTOZIP='.$entry['delivery']['zipcode'].
				'&PAYMENTREQUEST_0_SHIPTOCOUNTRYCODE='.$country.
				'&PAYMENTREQUEST_0_SHIPTOPHONENUM='.ltrim( str_replace( '-', '', $entry['delivery']['tel'] ), '0' );
			}
		}
...
?>
	<tr>
		...
		<td class="aleft"><?php echo apply_filters('usces_filter_admin_cart_item_name', esc_html($cartItemName), $materials ); ?><?php do_action('usces_admin_order_item_name', $order_id, $i); ?><?php usces_make_option_field( $materials, $cart ); ?></td>
...
function usces_get_link_key( $results ) {
	...
	if( isset($results['order_number']) && isset($results['sendpoint']) ) {
		$linkkey = $_REQUEST['sendpoint'];//zeus
	} elseif( isset($_REQUEST['X-S_TORIHIKI_NO']) ) {
	...
	} elseif( isset($_REQUEST['MerchantFree1']) ) {
		$linkkey = $_REQUEST['MerchantFree1'];//e-SCOTT
