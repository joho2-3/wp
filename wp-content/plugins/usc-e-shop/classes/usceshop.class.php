...
//		}
		...
		if( !get_option( 'usces_settlement_selected' ) ) {
			$settlement_selected = array();
			$payment_structure = array_keys( $this->payment_structure );
			...
			$settlement_selected = array_unique( $settlement_selected );
			if( 0 < count($settlement_selected) ) update_option( 'usces_settlement_selected', $settlement_selected );
		}
					...
					if( isset($_POST['settlement_selected']) ) {
						...
						foreach( (array)$acting_payments as $payment ) {
							if( !in_array( $payment, $settlement_selected ) ) {
								...
								$mes .= '※'.$settlement_name.'を未選択にすることはできません。支払方法を「削除」もしくは「停止」してください。<br />';
							}
						}
...
//					}
				...
				case 'jpayment':
					...
					if( WCUtils::is_blank($mes) ){
						...
						//	$this->payment_structure['acting_jpayment_webm'] = 'WebMoney決済（Cloud Payment）';
						//}else{
						//	unset($this->payment_structure['acting_jpayment_webm']);
						//}
						//if( 'on' == $options['acting_settings']['jpayment']['bitc_activate'] ){
						//	$this->payment_structure['acting_jpayment_bitc'] = 'BitCash決済（Cloud Payment）';
						//}else{
						//	unset($this->payment_structure['acting_jpayment_bitc']);
						//}
						//if( 'on' == $options['acting_settings']['jpayment']['suica_activate'] ){
						//	$this->payment_structure['acting_jpayment_suica'] = 'モバイルSuica決済（Cloud Payment）';
						//}else{
...
//		}
//		if( $this->is_cart_or_member_page($_SERVER['REQUEST_URI']) ) {
//			if( isset($this->options['address_search']) && $this->options['address_search'] == 'activate' ) {
...
?>
	<script type='text/javascript'>
		uscesL10n = {
			...
			'uscespage': "<?php echo $wcpage; ?>",
			'uscesid': "<?php echo $this->get_uscesid(false); ?>",
...
?>
		...
		/* <![CDATA[ */
			uscesL10n = {
				...
				'message': new Array( <?php echo $message; ?> ), 
	...
	function purchase(){
		...
		if( WCUtils::is_blank($this->error_message) && 0 < $this->cart->num_row()){
			...
			if( substr($payments['settlement'], 0, 6) == 'acting' && $entry['order']['total_full_price'] > 0 ){
				...
				//	if($key != 'purchase')
				//		$post_query .= '&' . $key . '=' . urlencode(maybe_serialize($value));
				//}
...
//			}
	...
	function is_purchased_item($mid, $post_id, $sku = NULL) {
		...
		return apply_filters( 'usces_filter_is_purchased_item', $res, $mid, $post_id, $sku );
	}
	...
	function incart_check() {
		...
		if( !empty($mes) ){
			...
			if( false === strpos($_POST['usces_referer'], 'http') ){
				...
				header('location: ' . $parse_url['scheme'] . '://' . $parse_url['host'] . esc_url($_POST['usces_referer']) . apply_filters('usces_filter_incart_redirect', '#cart_button', $post_id, $sku));
			}else{
	...
	function zaiko_check() {
		...
		for($i=0; $i<count($cart); $i++) { 
			...
			}else if( 1 < $zaiko_id || WCUtils::is_zero($stock) || 'publish' != $post_status ){
...
//	}
	...
	function getItemSkuAdvance( $post_id, $skukey = '' ){
		$array = array();
		$skus = $this->get_skus($post_id, 'code');
		foreach((array)$skus as $key => $sku){
	...
	function get_postIDbyCode( $itemcode ) {
		...
		$query = $wpdb->prepare( "SELECT post.ID FROM $wpdb->posts AS post 
			...
			AND (post.post_status = %s OR post.post_status = %s ) 
			LIMIT 1", 
			'_itemCode', $itemcode, 'publish', 'private' );
	...
	function order_processing( $results = array() ) {
		...
		//if($res != NULL && !in_array( $payments['settlement'], array('COD', 'installment', 'transferAdvance', 'transferDeferred'))) {
...
//usces_log('zeus conv header : '.print_r($header, true), 'acting_transaction.log');
			...
			$nvpstr .= '&CANCELURL='.apply_filters( 'usces_filter_paypal_ec_cancelurl', urlencode(USCES_CART_URL.$delim.'confirm=1'), $post_query );
			...
			$code = 0;
			$resBody = "";
			//$con = @fsockopen( 'tls://'.$url['host'], 443, $errno, $errstr, 30 );
...
//usces_log( print_r($res,true), "veritrans.log" );
			...
			if( !$fp ) {
				...
				$fp = fsockopen( 'ssl://'.$interface['host'], 443, $errno, $errstr, 30 );
				if( !$fp ) {
					usces_log( $acting.' : SSL Error', 'acting_transaction.log' );
					$log = array( 'acting'=>$acting, 'key'=>$trading_id, 'result'=>'SSL/TLS ERROR ('.$errno.')', 'data'=>array($errstr) );
					usces_save_order_acting_error( $log );
				...
				if( $data['result'] == 0 and !empty($data['url']) ) {
					if( $payment_type == "02" ) {
						...
						if( 'ordercompletion' == $res ) {
							...
							$this->set_order_meta_value( 'wc_trans_id', $trading_id, $order_id );
							$this->cart->crear_cart();
							header( "location: ".$data['url'] );
						} else {
							usces_log( $acting.'order error : '.print_r( $data, true ), 'acting_transaction.log' );
							$log = array( 'acting'=>$acting, 'key'=>$trading_id, 'result'=>'ORDER DATA REGISTERED ERROR', 'data'=>$data );
							usces_save_order_acting_error( $log );
							header( "location: ".USCES_CART_URL.$delim.'acting='.$acting.'&acting_return=0' );
						}
					}
					...
					$log = array( 'acting'=>$acting, 'key'=>$trading_id, 'result'=>'RESPONSE ERROR', 'data'=>$response );
					usces_save_order_acting_error( $log );
					header( "location: ".USCES_CART_URL.$delim.'acting='.$acting.'&acting_return=0' );
				}
				exit;
...
//usces_log('addr : '.$addr, 'acting_transaction.log');
	...
	function get_next_page_uri( $type, $current ){
		...
		if( !isset($_SESSION[$table]['currentPageIds']) )
			return false;
		
		$ids = $_SESSION[$table]['currentPageIds'];
