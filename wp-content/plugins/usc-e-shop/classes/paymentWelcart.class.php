...
class WELCARTPAY_SETTLEMENT
{
	...
	public function __construct() {
		...
		if( $this->is_activate_conv() || $this->is_activate_atodine() ) {
		...
		if( $this->is_activate_atodine() ) {
			if( is_admin() ) {
				add_action( 'usces_after_cart_instant', array( $this, 'atodine_upload' ), 9 );
				add_action( 'usces_action_order_list_page', array( $this, 'output_atodine_csv' ) );
				add_action( 'usces_action_order_list_searchbox_bottom', array( $this, 'action_atodine_button' ) );
				...
				add_filter( 'usces_order_list_action_status', array( $this, 'order_list_action_status' ), 9 );
				add_filter( 'usces_order_list_action_message', array( $this, 'order_list_action_message' ), 9 );

				$acting_opts = $this->get_acting_settings();
				if( isset($acting_opts['atodine_byitem']) && 'on' == $acting_opts['atodine_byitem'] ) {
					add_filter( 'usces_item_master_second_section', array( $this, 'edit_item_atodine_byitem' ), 10, 2 );
					add_action( 'usces_action_save_product', array( $this, 'save_item_atodine_byitem' ), 10, 2 );
				}
			} else {
	...
	***********************************************/
	public function initialize_data() {
		...
		if( !in_array( 'welcart', $options['acting_settings'] ) ) {
			...
			$options['acting_settings']['welcart']['operateid_dlseller'] = ( isset($options['acting_settings']['welcart']['operateid_dlseller']) ) ? $options['acting_settings']['welcart']['operateid_dlseller'] : '1Auth';
			...
			$options['acting_settings']['welcart']['atodine_activate'] = ( isset($options['acting_settings']['welcart']['atodine_activate']) ) ? $options['acting_settings']['welcart']['atodine_activate'] : 'off';
			$options['acting_settings']['welcart']['atodine_byitem'] = ( isset($options['acting_settings']['welcart']['atodine_byitem']) ) ? $options['acting_settings']['welcart']['atodine_byitem'] : 'off';
			$options['acting_settings']['welcart']['atodine_billing_method'] = ( isset($options['acting_settings']['welcart']['atodine_billing_method']) ) ? $options['acting_settings']['welcart']['atodine_billing_method'] : '2';
			$options['acting_settings']['welcart']['atodine_fee_type'] = ( isset($options['acting_settings']['welcart']['atodine_fee_type']) ) ? $options['acting_settings']['welcart']['atodine_fee_type'] : '';
			$options['acting_settings']['welcart']['atodine_fee'] = ( isset($options['acting_settings']['welcart']['atodine_fee']) ) ? $options['acting_settings']['welcart']['atodine_fee'] : '';
			$options['acting_settings']['welcart']['atodine_fee_limit_amount'] = ( isset($options['acting_settings']['welcart']['atodine_fee_limit_amount']) ) ? $options['acting_settings']['welcart']['atodine_fee_limit_amount'] : '';
			$options['acting_settings']['welcart']['atodine_fee_first_amount'] = ( isset($options['acting_settings']['welcart']['atodine_fee_first_amount']) ) ? $options['acting_settings']['welcart']['atodine_fee_first_amount'] : '';
			$options['acting_settings']['welcart']['atodine_fee_first_fee'] = ( isset($options['acting_settings']['welcart']['atodine_fee_first_fee']) ) ? $options['acting_settings']['welcart']['atodine_fee_first_fee'] : '';
			$options['acting_settings']['welcart']['atodine_fee_amounts'] = ( isset($options['acting_settings']['welcart']['atodine_fee_amounts']) ) ? $options['acting_settings']['welcart']['atodine_fee_amounts'] : array();
			$options['acting_settings']['welcart']['atodine_fee_fees'] = ( isset($options['acting_settings']['welcart']['atodine_fee_fees']) ) ? $options['acting_settings']['welcart']['atodine_fee_fees'] : array();
			$options['acting_settings']['welcart']['atodine_fee_end_fee'] = ( isset($options['acting_settings']['welcart']['atodine_fee_end_fee']) ) ? $options['acting_settings']['welcart']['atodine_fee_end_fee'] : '';
		...
		if( !in_array( 'acting_welcart_conv', $noreceipt_status ) || !in_array( 'acting_welcart_atodine', $noreceipt_status ) ) {
			$noreceipt_status[] = 'acting_welcart_conv';
			$noreceipt_status[] = 'acting_welcart_atodine';
	...
	***********************************************/
	function is_validity_welcart( $type = '' ) {
		...
		case 'atodine':
			foreach( $payment_method as $payment ) {
				if( 'acting_welcart_atodine' == $payment['settlement'] && 'activate' == $payment['use'] ) {
					$method = true;
					break;
				}
			}
			if( $method && $this->is_activate_atodine() ) {
				return true;
			} else {
				return false;
			}
	...
	* @return boolean $res
	***********************************************/
	public function is_activate_atodine() {

		$acting_opts = $this->get_acting_settings();
		if( ( isset($acting_opts['activate']) && 'on' == $acting_opts['activate'] ) && 
			( isset($acting_opts['atodine_activate']) && 'on' == $acting_opts['atodine_activate'] ) ) {
			$res = true;
		} else {
			$res = false;
		}
...
//usces_log("cancel=".$response_data['ResponseCd'],"test.log");
	...
	***********************************************/
	public function orderlist_settlement_status( $detail, $value, $key, $order_id ) {
		...
		case 'acting_welcart_card':
			...
			if( isset($latest_log['OperateId']) ) {
				...
				if( !empty($regular_id) && empty($trans_id) ) {
					$detail = '<td>'.$value.'<span class="acting-status card-error">'.__('カード未登録','usces').'</span></td>';
				}
			}
			break;
		...
		case 'acting_welcart_conv':
			...
			if( $expiration ) {
				$detail = '<td>'.$value.'<span class="acting-status conv-expiration">'.__('期限切れ','usces').'</span></td>';
			} else {
				$latest_log = $this->get_acting_latest_log( $order_id.'_'.$trans_id );
				if( isset($latest_log['OperateId']) && '2Del' == $latest_log['OperateId'] ) {
					$detail = '<td>'.$value.'<span class="acting-status conv-del">'.__('取消済み','usces').'</span></td>';
				} else {
	...
	***********************************************/
	public function settlement_info_field_value( $value, $key, $acting ) {
		...
		if( 'welcart_card' != $acting && 'welcart_conv' != $acting && 'welcart_atodine' != $acting ) {
			return $value;
		}
		...
		case 'acting':
			switch( $value ) {
			case 'welcart_card':
				$value = __('WelcartPay クレジットカード決済','usces');
				break;
			case 'welcart_conv':
				$value = __('WelcartPay オンライン収納代行','usces');
				break;
			case 'welcart_atodine':
				$value = __('WelcartPay 後払い決済','usces');
				break;
			}
			break;
			...
			if( in_array( $payment['settlement'], $this->pay_method ) ) {
				...
				if( !empty($MerchantFree1) ) {
					...
					if( isset($latest_log['OperateId']) ) {
						if( 'acting_welcart_conv' == $payment['settlement'] ) {
							...
							if( $expiration ) {
								...
								$status_name = __('期限切れ','usces');
							} else {
								if( '2Del' == $latest_log['OperateId'] ) {
									$class = ' conv-del';
									$status_name = __('取消済み','usces');
								}
							}
						} else {
						...
						if( !empty($status_name) ) {
							echo '
							<tr>
								<td class="label status">'.__('決済状況','usces').'</td>
					...
					if( !empty($regular_id) ) {
						...
						<tr>
							<td class="label status">'.__('決済状況','usces').'</td>
							<td class="col1 status"><span id="settlement-status"><span class="acting-status card-error">'.__('カード未登録','usces').'</span></span></td>
						</tr>';
					}
				}
			}
	...
	public function settlement_information( $data, $action_args ) {
		...
		if( $order_action != 'new' && !empty($order_id) ) {
			...
			if( in_array( $payment['settlement'], $this->pay_method ) ) {
				...
				//	echo '<input type="button" id="settlement-information-'.$acting_data['MerchantFree1'].'-1" class="button settlement-information" value="'.__('決済情報','usces').'">';
				//}
				echo '<input type="button" id="settlement-information-'.$MerchantFree1.'-1" class="button settlement-information" value="'.__('決済情報','usces').'">';
			}
		}
	}
...
<?php
	...
	***********************************************/
	public function revival_order_metadata( $order_id, $log_key, $acting ) {
		...
		if( 'acting_welcart_conv' == $acting ) {
			$acting_opts = $this->get_acting_settings();
			$paylimit = date( 'Ymd', strtotime($order_data['order_date'])+(86400*$acting_opts['conv_limit']) ).'2359';
	...
	***********************************************/
	public function check_acting_return_duplicate( $trans_id, $results ) {
		...
		if( !$entry['order']['total_full_price'] ) {
			return 'not_credit';
		} elseif( isset($results['MerchantFree1']) && isset($results['acting']) && ( 'welcart_card' == $results['acting'] || 'welcart_conv' == $results['acting'] || 'welcart_atodine' == $results['acting'] ) ) {
...
(function($) {
	$(document).on( "click", "#card-delete", function() {
		if( confirm("<?php _e('クレジットカードの登録を削除します。よろしいですか？','usces'); ?>") ) {
	...
	public function delivery_secure_form( $nouse, $payment ) {
		...
		case 'acting_welcart_card':
			...
			if( 'OK' == $response_member['ResponseCd'] && !$backDelivery ) {
				...
				$expyy = substr(date('Y', current_time('timestamp')), 0, 2).substr($response_member['CardExp'], 0, 2);
				...
				<tr>
					...
					<td colspan="2"><p>'.$cardlast4.' (<a href="'.add_query_arg( array('backDelivery'=>'welcart_card'), USCES_CART_URL ).'">'.__('Change of card information, click here','usces').'</a>)</p></td>
				</tr>';
				...
				<tr>
					...
					<td colspan="2"><input name="cardno" type="text" id="welcart_cnum" size="16" value="'.$cardno.'" />'.__('(Single-byte numbers only)','usces').'</td>
				</tr>';
				if( 'on' == $acting_opts['seccd'] ) {
				...
				<tr>
					...
					<td colspan="2"><input name="seccd" type="text" size="6" value="'.$seccd.'" />'.__('(Single-byte numbers only)','usces').'</td>
				</tr>';
				}
				$html .= '
				<tr>
...
//usces_log(print_r($post_data,true),"test.log");
		...
		case 'acting_welcart_card':
			...
			if( 'on' == $acting_opts['card_activate'] ) {
				...
				$response_data = $this->connection( $params );
				$response_data['acting'] = $acting;
				$response_data['PayType'] = $post_data['paytype'];
		...
		case 'acting_welcart_conv':
			...
			$item_name = mb_convert_kana($usces->getItemName($cart[0]['post_id']), 'ASK');
			if( 1 < count($cart) ) {
				if( 11 < mb_strlen($item_name.__(' etc.','usces')) ) {
					$item_name = mb_substr($item_name, 0, 10).__(' etc.','usces');
				}
			} else {
				if( 11 < mb_strlen($item_name) ) {
					$item_name = mb_substr($item_name, 0, 10).__('...','usces');
				}
			}
			$paylimit = date( 'Ymd', current_time('timestamp')+(86400*$acting_opts['conv_limit']) ).'2359';
	...
	***********************************************/
	public function settlement_update() {
		...
		$options['acting_settings']['welcart']['atodine_activate'] = ( isset($_POST['atodine_activate']) ) ? $_POST['atodine_activate'] : '';
/*		$options['acting_settings']['welcart']['atodine_byitem'] = ( isset($_POST['atodine_byitem']) ) ? $_POST['atodine_byitem'] : 'off';
		$options['acting_settings']['welcart']['atodine_billing_method'] = ( isset($_POST['atodine_billing_method']) ) ? $_POST['atodine_billing_method'] : '2';
		$options['acting_settings']['welcart']['atodine_fee_type'] = ( isset($_POST['atodine_fee_type']) ) ? $_POST['atodine_fee_type'] : '';
		$options['acting_settings']['welcart']['atodine_fee'] = ( isset($_POST['atodine_fee']) ) ? $_POST['atodine_fee'] : '';
		$options['acting_settings']['welcart']['atodine_fee_limit_amount'] = ( isset($_POST['atodine_fee_limit_amount']) ) ? $_POST['atodine_fee_limit_amount'] : '';
		$options['acting_settings']['welcart']['atodine_fee_first_amount'] = ( isset($_POST['atodine_fee_first_amount']) ) ? $_POST['atodine_fee_first_amount'] : '';
		$options['acting_settings']['welcart']['atodine_fee_first_fee'] = ( isset($_POST['atodine_fee_first_fee']) ) ? $_POST['atodine_fee_first_fee'] : '';
		$options['acting_settings']['welcart']['atodine_fee_amounts'] = ( isset($_POST['atodine_fee_amounts']) ) ? explode( '|', $_POST['atodine_fee_amounts'] ) : array();
		$options['acting_settings']['welcart']['atodine_fee_fees'] = ( isset($_POST['atodine_fee_fees']) ) ? explode( '|', $_POST['atodine_fee_fees'] ) : array();
		$options['acting_settings']['welcart']['atodine_fee_end_fee'] = ( isset($_POST['atodine_fee_end_fee']) ) ? $_POST['atodine_fee_end_fee'] : '';
*/
		if( WCUtils::is_blank($_POST['merchant_id']) ) {
			$this->error_mes .= __('※マーチャントIDを入力してください<br />','usces');
		}
		if( WCUtils::is_blank($_POST['merchant_pass']) ) {
			$this->error_mes .= __('※マーチャントパスワードを入力してください<br />','usces');
		}
		if( WCUtils::is_blank($_POST['tenant_id']) ) {
			$this->error_mes .= __('※店舗コードを入力してください<br />','usces');
		}
		if( WCUtils::is_blank($_POST['auth_key']) ) {
			$this->error_mes .= __('※決済認証キーを入力してください<br />','usces');
		} else {
			$auth_key = md5($_POST['auth_key']);
			$welcartpay_keys = get_option( 'usces_welcartpay_keys' );
			if( !in_array( $auth_key, $welcartpay_keys ) ) {
				$this->error_mes .= __('※決済認証キーが正しくありません<br />','usces');
			}
		}
		if( WCUtils::is_blank($_POST['ope']) ) {
			$this->error_mes .= __('※動作環境を選択してください<br />','usces');
		}
		if( 'on' == $options['acting_settings']['welcart']['card_activate'] ) {
			...
			if( $unavailable_activate ) {
				$this->error_mes .= __('※併用できない決済が有効化されています<br />','usces');
			}
		}
		...
		if( WCUtils::is_blank($this->error_mes) ) {
			...
			if( 'public' == $options['acting_settings']['welcart']['ope'] ) {
				...
				$options['acting_settings']['welcart']['key_aes'] = 'HgmhZ94rN799CD3F';
				$options['acting_settings']['welcart']['key_iv'] = 'gNqc4zwhNLCSC5cv';
			} else {
				...
				$options['acting_settings']['welcart']['key_aes'] = 'sonypaysonypay11';
				$options['acting_settings']['welcart']['key_iv'] = 'sonypaysonypay22';
			...
			if( 'on' == $options['acting_settings']['welcart']['card_activate'] || 'link' == $options['acting_settings']['welcart']['card_activate'] ) {
				$usces->payment_structure['acting_welcart_card'] = __('カード決済（WelcartPay）','usces');
			} else {
				unset($usces->payment_structure['acting_welcart_card']);
			}
			if( 'on' == $options['acting_settings']['welcart']['conv_activate'] ) {
				$usces->payment_structure['acting_welcart_conv'] = __('オンライン収納代行（WelcartPay）','usces');
			} else {
				unset($usces->payment_structure['acting_welcart_conv']);
			}
			if( 'on' == $options['acting_settings']['welcart']['atodine_activate'] ) {
				$usces->payment_structure['acting_welcart_atodine'] = __('後払い決済（WelcartPay）','usces');
			} else {
				unset($usces->payment_structure['acting_welcart_atodine']);
			}
		} else {
...
?>
	...
	<form action="" method="post" name="welcart_form" id="welcart_form">
		<table class="settle_table">
			<tr>
				<th><a style="cursor:pointer;" onclick="toggleVisibility('ex_merchant_id_welcart');"><?php _e('マーチャントID','usces'); ?></a></th>
				<td colspan="4"><input name="merchant_id" type="text" id="merchant_id_welcart" value="<?php echo $acting_opts['merchant_id']; ?>" size="20" /></td>
				<td><div id="ex_merchant_id_welcart" class="explanation"><?php _e('契約時に e-SCOTT から発行されるマーチャントID（半角数字）','usces'); ?></div></td>
			</tr>
			<tr>
				<th><a style="cursor:pointer;" onclick="toggleVisibility('ex_merchant_pass_welcart');"><?php _e('マーチャントパスワード','usces'); ?></a></th>
				...
				<td><div id="ex_merchant_pass_welcart" class="explanation"><?php _e('契約時に e-SCOTT から発行されるサービスパスワード（半角英数字）','usces'); ?></div></td>
			</tr>
			<tr>
				<th><a style="cursor:pointer;" onclick="toggleVisibility('ex_tenant_id_welcart');"><?php _e('店舗コード','usces'); ?></a></th>
				<td colspan="4"><input name="tenant_id" type="text" id="tenant_id_welcart" value="<?php echo $acting_opts['tenant_id']; ?>" size="20" /></td>
				<td><div id="ex_tenant_id_welcart" class="explanation"><?php _e('契約時に e-SCOTT から発行される店舗コード<br/>契約するショップが1つだけの場合は、0001 と入力してください。','usces'); ?></div></td>
			</tr>
			<tr>
				<th><a style="cursor:pointer;" onclick="toggleVisibility('ex_auth_key_welcart');"><?php _e('決済認証キー','usces'); ?></a></th>
				<td colspan="4"><input name="auth_key" type="text" id="auth_key_welcart" value="<?php echo $acting_opts['auth_key']; ?>" size="20" /></td>
				<td><div id="ex_auth_key_welcart" class="explanation"><?php _e('契約時に e-SCOTT から発行される決済認証キー（半角数字）','usces'); ?></div></td>
			</tr>
			<tr>
				<th><a style="cursor:pointer;" onclick="toggleVisibility('ex_ope_welcart');"><?php _e('Operation Environment','usces'); ?></a></th>
				<td><input name="ope" type="radio" id="ope_welcart_1" value="test"<?php if( $acting_opts['ope'] == 'test' ) echo ' checked="checked"'; ?> /></td><td><label for="ope_welcart_1"><?php _e('テスト環境','usces'); ?></label></td>
				<td><input name="ope" type="radio" id="ope_welcart_2" value="public"<?php if( $acting_opts['ope'] == 'public' ) echo ' checked="checked"'; ?> /></td><td><label for="ope_welcart_2"><?php _e('本番環境','usces'); ?></label></td>
				<td><div id="ex_ope_welcart" class="explanation"><?php _e('動作環境を切り替えます。','usces'); ?></div></td>
			</tr>
		</table>
		<table class="settle_table">
			<tr>
				<th><?php _e('クレジットカード決済','usces'); ?></th>
				<td><input name="card_activate" type="radio" class="card_activate_welcart" id="card_activate_welcart_1" value="on"<?php if( $acting_opts['card_activate'] == 'on' ) echo ' checked="checked"'; ?> /></td><td><label for="card_activate_welcart_1"><?php _e('埋め込み型で利用する','usces'); ?></label></td>
				<td><input name="card_activate" type="radio" class="card_activate_welcart" id="card_activate_welcart_2" value="link"<?php if( $acting_opts['card_activate'] == 'link' ) echo ' checked="checked"'; ?> /></td><td><label for="card_activate_welcart_2"><?php _e('外部リンク型で利用する','usces'); ?></label></td>
			...
			<!--<tr class="card_welcart">
				<th><a style="cursor:pointer;" onclick="toggleVisibility('ex_foreign_activate_welcart');"><?php _e('外貨決済','usces'); ?></a></th>
				...
				<td></td><td></td>
				<td><div id="ex_foreign_activate_welcart" class="explanation"><?php _e('外貨決済では、利用可能なカード会社（ブランド）は、VISA および MasterCard の 2社のみとなります。<br/>運用の途中で通常（円貨カード）との切り替えをされますと、e-SCOTT に登録した会員情報が無効になります。','usces'); ?></div></td>
			</tr>-->
			<tr class="card_welcart">
				<th><a style="cursor:pointer;" onclick="toggleVisibility('ex_seccd_welcart');"><?php _e('セキュリティコード<br />（認証アシスト）','usces'); ?></a></th>
				...
				<td></td><td></td>
				<td><div id="ex_seccd_welcart" class="explanation"><?php _e('認証アシストマッチングの「セキュリティコード」を利用します。「利用しない」にする場合、e-SCOTT の管理画面でも「マッチング検証しない」に設定してください。','usces'); ?></div></td>
			</tr>
			<tr class="card_welcart">
				<th><a style="cursor:pointer;" onclick="toggleVisibility('ex_quickpay_welcart');"><?php _e('クイック決済','usces'); ?></a></th>
				...
				<td></td><td></td>
				<td><div id="ex_quickpay_welcart" class="explanation"><?php _e('自動継続課金（要 WCEX DLSeller）もしくは定期購入（要 WCEX Auto Delivery）を利用する場合、クイック決済を「利用する」にしてください。','usces'); ?></div></td>
			</tr>
			<tr class="card_welcart">
				<th><?php _e('処理区分','usces'); ?></th>
				<td><input name="operateid" type="radio" id="operateid_welcart_1" value="1Auth"<?php if( $acting_opts['operateid'] == '1Auth' ) echo ' checked="checked"'; ?> /></td><td><label for="operateid_welcart_1"><?php _e('与信','usces'); ?></label></td>
				<td><input name="operateid" type="radio" id="operateid_welcart_2" value="1Gathering"<?php if( $acting_opts['operateid'] == '1Gathering' ) echo ' checked="checked"'; ?> /></td><td><label for="operateid_welcart_2"><?php _e('与信売上計上','usces'); ?></label></td>
				<td></td><td></td><td></td>
			</tr>
			<?php if( defined('WCEX_DLSELLER') ): ?>
			<tr class="card_welcart">
				<th><a style="cursor:pointer;" onclick="toggleVisibility('ex_operateid_dlseller_welcart');"><?php _e('自動継続課金処理区分','usces'); ?></a></th>
				<td><input name="operateid_dlseller" type="radio" id="operateid_dlseller_welcart_1" value="1Auth"<?php if( $acting_opts['operateid_dlseller'] == '1Auth' ) echo ' checked="checked"'; ?> /></td><td><label for="operateid_dlseller_welcart_1"><?php _e('与信','usces'); ?></label></td>
				<td><input name="operateid_dlseller" type="radio" id="operateid_dlseller_welcart_2" value="1Gathering"<?php if( $acting_opts['operateid_dlseller'] == '1Gathering' ) echo ' checked="checked"'; ?> /></td><td><label for="operateid_dlseller_welcart_2"><?php _e('与信売上計上','usces'); ?></label></td>
				<td></td><td></td>
				<td><div id="ex_operateid_dlseller_welcart" class="explanation"><?php _e('自動継続課金（要 WCEX DLSeller）時の処理区分。','usces'); ?></div></td>
			</tr>
			<tr class="card_welcart">
				<th><a style="cursor:pointer;" onclick="toggleVisibility('ex_auto_settlement_mail_welcart');"><?php _e('自動継続課金完了メール','usces'); ?></a></th>
				...
				<td></td><td></td>
				<td><div id="ex_auto_settlement_mail_welcart" class="explanation"><?php _e('自動継続課金処理（要 WCEX DLSeller）が実行された会員に、課金完了メールを送信します。','usces'); ?></div></td>
			</tr>
			<?php endif; ?>
			<tr class="card_howtopay_welcart">
				<th><a style="cursor:pointer;" onclick="toggleVisibility('ex_howtopay_welcart');"><?php _e('Number of payments','usces'); ?></a></th>
				<td><input name="howtopay" type="radio" id="howtopay_welcart_1" value="1"<?php if( $acting_opts['howtopay'] == '1' ) echo ' checked="checked"'; ?> /></td><td><label for="howtopay_welcart_1"><?php _e('一括払いのみ','usces'); ?></label></td>
				<td><input name="howtopay" type="radio" id="howtopay_welcart_2" value="2"<?php if( $acting_opts['howtopay'] == '2' ) echo ' checked="checked"'; ?> /></td><td><label for="howtopay_welcart_2"><?php _e('分割払いを有効にする','usces'); ?></label></td>
				<td><input name="howtopay" type="radio" id="howtopay_welcart_3" value="3"<?php if( $acting_opts['howtopay'] == '3' ) echo ' checked="checked"'; ?> /></td><td><label for="howtopay_welcart_3"><?php _e('分割払いとボーナス払いを有効にする','usces'); ?></label></td>
				<td><div id="ex_howtopay_welcart" class="explanation"><?php _e('埋め込み型で利用するときに選択できます。','usces'); ?></div></td>
			</tr>
		</table>
		<table class="settle_table">
			<tr>
				<th><?php _e('オンライン収納代行','usces'); ?></th>
			...
			<tr class="conv_welcart">
				<th><?php _e('支払期限日数','usces'); ?></th>
				<td colspan="4"><input name="conv_limit" type="text" id="conv_limit" value="<?php echo $acting_opts['conv_limit']; ?>" size="5" /><?php _e('日','usces'); ?></td>
				<td></td>
			</tr>
			<tr class="conv_welcart">
				<th><a style="cursor:pointer;" onclick="toggleVisibility('ex_conv_fee_welcart');"><?php _e('Fee','usces'); ?></a></th>
				...
				<td><div id="ex_conv_fee_welcart" class="explanation"><?php _e('オンライン収納代行手数料と決済上限額を設定します。必要ない場合は空白にしておきます。','usces'); ?></div></td>
			</tr>
		</table>
		<!--<table class="settle_table">
			<tr>
				<th><?php _e('後払い決済','usces'); ?></th>
				<td><input name="atodine_activate" type="radio" class="atodine_activate_welcart" id="atodine_activate_welcart_1" value="on"<?php if( $acting_opts['atodine_activate'] == 'on' ) echo ' checked="checked"'; ?> /></td><td><label for="atodine_activate_welcart_1"><?php _e('Use','usces'); ?></label></td>
				<td><input name="atodine_activate" type="radio" class="atodine_activate_welcart" id="atodine_activate_welcart_0" value="off"<?php if( $acting_opts['atodine_activate'] == 'off' ) echo ' checked="checked"'; ?> /></td><td><label for="atodine_activate_welcart_0"><?php _e('Do not Use','usces'); ?></label></td>
				<td></td>
			</tr>
			<tr class="atodine_welcart">
				<th><a style="cursor:pointer;" onclick="toggleVisibility('ex_atodine_byitem_welcart');"><?php _e('商品ごとの可否','usces'); ?></a></th>
				<td><input name="atodine_byitem" type="radio" id="atodine_byitem_welcart_1" value="on"<?php if( $acting_opts['atodine_byitem'] == 'on' ) echo ' checked="checked"'; ?> /></td><td><label for="atodine_byitem_welcart_1"><?php _e("enable",'usces'); ?></label></td>
				<td><input name="atodine_byitem" type="radio" id="atodine_byitem_welcart_0" value="off"<?php if( $acting_opts['atodine_byitem'] == 'off' ) echo ' checked="checked"'; ?> /></td><td><label for="atodine_byitem_welcart_0"><?php _e("disable",'usces'); ?></label></td>
				<td><div id="ex_atodine_byitem_welcart" class="explanation"><?php _e('商品ごとに後払い決済の可否を設定する場合は有効。特に区別しない場合は無効。<br />有効にした場合は、商品登録画面に後払い決済ができるかどうかの選択フィールドが追加されます。後払い決済ができない商品がカートにあった場合、支払方法の選択肢から後払い決済を除外します。<br />また、商品CSVにはカスタムフィールド（welcartpay_atodine）として可否データが追加されます。','usces'); ?></div></td>
			</tr>
			<tr class="atodine_welcart">
				<th><a style="cursor:pointer;" onclick="toggleVisibility('ex_atodine_billing_method_welcart');"><?php _e('請求書送付方法','usces'); ?></a></th>
				<td><input name="atodine_billing_method" type="radio" id="atodine_billing_method_welcart_2" value="2"<?php if( $acting_opts['atodine_billing_method'] == '2' ) echo ' checked="checked"'; ?> /></td><td><label for="atodine_billing_method_welcart_2"><?php _e('別送','usces'); ?></label></td>
				<td><input name="atodine_billing_method" type="radio" id="atodine_billing_method_welcart_3" value="3"<?php if( $acting_opts['atodine_billing_method'] == '3' ) echo ' checked="checked"'; ?> /></td><td><label for="atodine_billing_method_welcart_3"><?php _e('同梱','usces'); ?></label></td>
				<td><div id="ex_atodine_billing_method_welcart" class="explanation"><?php _e('ATODINEからの請求書送付方法','usces'); ?></div></td>
			</tr>
			<tr class="atodine_welcart">
				<th><a style="cursor:pointer;" onclick="toggleVisibility('ex_atodine_fee_welcart');"><?php _e('Fee','usces'); ?></a></th>
				<td colspan="2" id="atodine_fee_type_field"><?php echo $this->get_fee_name( $acting_opts['atodine_fee_type'] ); ?></td><td colspan="2"><input type="button" class="button" value="<?php _e('Detailed setting','usces'); ?>" id="atodine_fee_setting" /></td>
				<td><div id="ex_atodine_fee_welcart" class="explanation"><?php _e('後払い決済手数料と決済上限額を設定します。必要ない場合は空白にしておきます。','usces'); ?></div></td>
			</tr>
		</table>-->
		...
		<input type="hidden" name="atodine_fee_type" id="atodine_fee_type" value="<?php echo $acting_opts['atodine_fee_type']; ?>" />
		<input type="hidden" name="atodine_fee" id="atodine_fee" value="<?php echo $acting_opts['atodine_fee']; ?>" />
		<input type="hidden" name="atodine_fee_limit_amount_fix" id="atodine_fee_limit_amount_fix" value="<?php echo $acting_opts['atodine_fee_limit_amount']; ?>" />
		<input type="hidden" name="atodine_fee_first_amount" id="atodine_fee_first_amount" value="<?php echo $acting_opts['atodine_fee_first_amount']; ?>" />
		<input type="hidden" name="atodine_fee_first_fee" id="atodine_fee_first_fee" value="<?php echo $acting_opts['atodine_fee_first_fee']; ?>" />
		<input type="hidden" name="atodine_fee_limit_amount_change" id="atodine_fee_limit_amount_change" value="<?php echo $acting_opts['atodine_fee_limit_amount']; ?>" />
		<input type="hidden" name="atodine_fee_amounts" id="atodine_fee_amounts" value="<?php echo implode('|', $acting_opts['atodine_fee_amounts']); ?>" />
		<input type="hidden" name="atodine_fee_fees" id="atodine_fee_fees" value="<?php echo implode('|', $acting_opts['atodine_fee_fees']); ?>" />
		<input type="hidden" name="atodine_fee_end_fee" id="atodine_fee_end_fee" value="<?php echo $acting_opts['atodine_fee_end_fee']; ?>" />
		<input name="usces_option_update" type="submit" class="button button-primary" value="<?php _e('WelcartPay の設定を更新する','usces'); ?>" />
		<?php wp_nonce_field( 'admin_settlement', 'wc_nonce' ); ?>
	</form>
	<div class="settle_exp">
		<p><strong>WelcartPay based on e-SCOTT</strong></p>
		<a href="http://www.sonypaymentservices.jp/intro/" target="_blank">e-SCOTT Smart の詳細はこちら 》</a>
		<p>&nbsp;</p>
		<p>「埋め込み型」とは、決済会社のページへは遷移せず、Welcart のページのみで完結する決済システムです。<br />
			デザインの統一されたスタイリッシュな決済が可能です。但し、カード番号を扱いますので専用SSLが必須となります。<br />
			「外部リンク型」とは、決済会社のページへ遷移してカード情報を入力する決済システムです。</p>
		<p>いずれも、入力されたカード番号は e-SCOTT Smart のシステムに送信されますので、Welcart に保存することはありません。</p>
		<p>「自動継続課金」を利用するには「WCEX DL Seller」拡張プラグインのインストールが必要です。<br />
			「定期購入」を利用するには「WCEX Auto Delivery」拡張プラグインのインストールが必要です。</p>
		<p>尚、本番環境では、正規SSL証明書のみでのSSL通信となりますのでご注意ください。</p>
		<p>テスト環境で利用した Welcart 会員アカウントは、本番環境で利用できない場合があります。<br/>
			テスト環境と本番環境で別の会員登録を行うか、テスト環境で利用した会員を一旦削除してから、本番環境で改めて会員登録してください。</p>
	</div>
...
<?php			if( !usces_have_member_continue_order( $data['ID'] ) && !usces_have_member_regular_order( $data['ID'] ) ): ?>
		<tr>
			...
			<td><label for="welcart-quickpay-release"><?php _e('クイック決済を解除する','usces'); ?></label></td>
		</tr>
<?php			endif;
			endif;
		endif;
		...
		if( 'acting_welcart_card' == $payment['settlement'] ) {
			...
			if( 'link' == $acting_opts['card_activate'] ) {
				if( usces_is_login() && 'on' == $acting_opts['quickpay'] ) {
					...
					$KaiinPass = $this->get_quick_pass( $member['ID'] );
					if( !empty($KaiinId) && !empty($KaiinPass) ) {
						$quickpay = '<p class="welcartpay_quick_member"><label type="update"><input type="checkbox" name="quick_member" value="update"><span>'.__('登録済のクレジットカードを変更して購入する','usces').'</span></label></p>';
					} else {
						if( usces_have_regular_order() || usces_have_continue_charge() ) {
							$quickpay = '<input type="hidden" name="quick_member" value="add">';
						} else {
							$quickpay = '<p class="welcartpay_quick_member"><label type="add"><input type="checkbox" name="quick_member" value="add"><span>'.__('クレジットカードを登録して購入する','usces').'</span></label></p>';
						}
					}
				} else {
		...
		if( 'link' == $acting_opts['card_activate'] ) {
			...
			if( 'on' == $acting_opts['quickpay'] ) {
				...
				/*} elseif( isset($_POST['delete']) && 'delete' == $_POST['delete'] ) {
					...
					if( 'OK' == $response_member['ResponseCd'] ) {
						$message = __('クレジットカードの登録を削除しました。','usces');
						$deleted = true;
					} else {
						$error_message = array();
						$responsecd = explode( '|', $response_member['ResponseCd'] );
...
<?php
	...
	***********************************************/
	public function continue_member_settlement_info_page( $member_id, $order_id ) {
		...
		$year = substr(get_date_from_gmt(gmdate('Y-m-d H:i:s', time())), 0, 4);
		$total_full_price = $order_data['order_item_total_price'] - $order_data['order_usedpoint'] + $order_data['order_discount'] + $order_data['order_shipping_charge'] + $order_data['order_cod_fee'] + $order_data['order_tax'];
		...
		$KaiinId = $this->get_quick_kaiin_id( $member_id );
		$card = ( empty($KaiinId) ) ? '&nbsp;<span id="settlement-status"><span class="acting-status card-error">'.__('カード未登録','usces').'</span></span>' : '';
?>
<div class="wrap">
...
<div id="searchBox" style="display:block">
	...
	<tr>
		...
		<td>
			...
			<tr>
				<th><?php _e('受注時金額','usces'); ?></th>
				<td><?php usces_crform( $total_full_price, false ); ?></td>
				<th><?php _e('決済金額','usces'); ?></th>
				<td><input type="text" id="price" style="text-align: right;" value="<?php echo $continue_data['price']; ?>"><?php usces_crcode(); ?></td>
			</tr>
			<tr>
...
<table id="mainDataTable" class="new-table order-new-table">
	...
	<tr>
		<th scope="col">&nbsp;</th>
		<th scope="col"><?php _e('処理日時','usces'); ?></th>
		<th scope="col"><?php _e('決済ID','usces'); ?></th>
		<th scope="col"><?php _e('処理区分','usces'); ?></th>
		<th scope="col">&nbsp;</th>
	</tr>
	</thead>
...
?>
	...
	<tr>
		...
		<td><?php echo $log['MerchantFree1']; ?></td>
		...
		<td><input type="button" id="settlement-information-<?php echo $log['MerchantFree1']; ?>-<?php echo $num; ?>" class="button settlement-information" value="<?php _e('決済情報','usces'); ?>"></td>
		<?php else: ?>
		<td>&nbsp;</td><td>&nbsp;</td>
		<?php endif; ?>
	</tr>
...
?>
	...
	<tr>
		...
		<?php if( !empty($status_name) ): ?>
		<td><span id="settlement-status"><span class="acting-status<?php echo $class; ?>"><?php echo $status_name; ?></span></span></td>
		<td><input type="button" id="settlement-information-<?php echo $trans_id; ?>-1" class="button settlement-information" value="<?php _e('決済情報','usces'); ?>"></td>
		<?php else: ?>
		<td>&nbsp;</td><td>&nbsp;</td>
		<?php endif; ?>
	</tr>
	...
	public function continue_member_card_update_mail( $message, $member_id, $order_id ) {
		...
		if( !empty($KaiinId) && !empty($KaiinPass) ) {
			...
			if( 'OK' == $response_member['ResponseCd'] ) {
				...
				$message .= "今月中に下記の URL をクリックしてカード情報の更新を行なっていただきたいと\n";
				$message .= "存じます。\n";
...
//usces_log(print_r($response_data,true),"test.log");
	...
	***********************************************/
	function auto_settlement_mail( $member_id, $order_id, $response_data, $continue_data ) {
		...
		if( 'on' == $acting_opts['auto_settlement_mail'] ) {
			$subject = __('自動継続課金処理のお知らせ','usces');
			$name = usces_localized_name( $order_data['order_name1'], $order_data['order_name2'], 'return' );
			$mail_data = $usces->options['mail_data'];
			$mail_header = __('以下の通り、自動課金処理を行いましたのでご報告いたします。','usces')."\r\n\r\n";
			$mail_footer = __('ご不明な点はショップにお問い合わせくださいませ。','usces')."\r\n\r\n".$mail_data['footer']['thankyou'];
	...
	***********************************************/
	function auto_settlement_error_mail( $member_id, $order_id, $response_data, $continue_data ) {
		...
		if( 'on' == $acting_opts['auto_settlement_mail'] ) {
			...
			$name = usces_localized_name( $order_data['order_name1'], $order_data['order_name2'], 'return' );
			$mail_data = $usces->options['mail_data'];
			$mail_header = __('自動課金処理にてエラーが発生いたしましたのでご報告いたします。','usces')."\r\n\r\n";
			$mail_footer = __('ご不明な点はショップにお問い合わせくださいませ。','usces')."\r\n\r\n".$mail_data['footer']['thankyou'];
			$message = $mail_header.$mail_body.$mail_footer;
			if( isset($usces->options['put_customer_name']) && $usces->options['put_customer_name'] == 1 ) {
				$dear_name = sprintf( __('Dear %s','usces'), $name );
	...
	***********************************************/
	function auto_settlement_message( $member_id, $order_id, $order_data, $response_data, $continue_data ) {
		...
		$message .= __('Member ID','dlseller').' : '.$member_id."\r\n";
		$message .= __('Contractor name','dlseller').' : '.sprintf( _x('%s','honorific','usces'), $name )."\r\n";
		$message .= '決済金額 : '.usces_crform( $continue_data['price'], true, false, 'return' )."\r\n";
		if( isset($response_data['MerchantFree1']) ) {
			$message .= '決済ID : '.$response_data['MerchantFree1']."\r\n";
		}
		if( isset($response_data['TransactionId']) ) {
			$message .= '処理通番 : '.$response_data['TransactionId']."\r\n";
		}
		if( !empty($charged_date) ) {
	...
	***********************************************/
	public function do_auto_continuation( $today, $todays_charging ) {
		...
		$ok = ( empty($this->continuation_charging_mail['OK']) ) ? 0 : $this->continuation_charging_mail['OK'];
		$error = ( empty($this->continuation_charging_mail['NG']) ) ? 0 : $this->continuation_charging_mail['NG'];
		$admin_subject = '自動継続課金処理結果 '.$today;
		$admin_message = '自動継続課金処理が完了しましたのでご報告いたします。'."\r\n\r\n"
			.'処理日時：'.date_i18n( 'Y-m-d H:i:s', current_time('timestamp') )."\r\n"
			.'正常：'.$ok.' 件'."\r\n"
			.'異常：'.$error.' 件'."\r\n\r\n";
		foreach( $this->continuation_charging_mail['mail'] as $mail ) {
			$admin_message .= $mail."\r\n";
		}
		$admin_message .= '詳細は管理画面＞継続課金会員リスト＞継続課金会員情報画面にてご確認ください。'."\r\n";
...
jQuery(document).ready( function($) {
	...
	var atodine_activate = "<?php echo $acting_opts['atodine_activate']; ?>";
	if( "on" == card_activate ) {
	...
	if( "on" == atodine_activate ) {
		$(".atodine_welcart").css("display", "");
	} else {
		$(".atodine_welcart").css("display", "none");
	}
	...
	$(document).on( "change", ".atodine_activate_welcart", function() {
		if( "on" == $( this ).val() ) {
			$(".atodine_welcart").css("display", "");
		} else {
			$(".atodine_welcart").css("display", "none");
	...
	$(document).on("click", "#conv_fee_setting", function() {
		...
		$("#welcartpay_fee_dialog").dialog( "option", "title", "<?php _e('オンライン収納代行決済手数料設定','usces'); ?>" );
		$("#welcartpay_fee_dialog").dialog( "open" );
	});

	$(document).on("click", "#atodine_fee_setting", function() {
		$("#welcartpay_fee_mode").val( "atodine" );
		$("#welcartpay_fee_dialog").dialog( "option", "title", "<?php _e('後払い決済手数料設定','usces'); ?>" );
		$("#welcartpay_fee_dialog").dialog( "open" );
	});
	...
	adminSettlementWelcartPay.setFeeType( "atodine", false );
});
</script>
<?php
			endif;
...
jQuery(document).ready( function($) {
	...
	$(document).on("click", "#capture-settlement", function() {
		if( !confirm("<?php _e('売上計上処理を実行します。よろしいですか？','usces'); ?>") ) {
			return;
		}
	...
	$(document).on("click", "#delete-settlement", function() {
		if( !confirm("<?php _e('取消処理を実行します。よろしいですか？','usces'); ?>") ) {
			return;
		}
	...
	$(document).on("click", "#change-settlement", function() {
		...
		if( amount == "" || parseInt(amount) === 0 || !checkNum(amount) ) {
			alert("<?php _e('利用金額の形式が不正です。数値で入力してください。','usces'); ?>");
			return;
		}
		if( !confirm("<?php _e('利用金額の変更処理を実行します。よろしいですか？','usces'); ?>") ) {
			return;
		}
	...
	$(document).on("click", "#auth-settlement", function() {
		...
		if( !confirm("<?php _e('与信処理を実行します。よろしいですか？','usces'); ?>") ) {
			return;
		}
	...
	$(document).on("click", "#gathering-settlement", function() {
		...
		if( !confirm("<?php _e('与信売上計上処理を実行します。よろしいですか？','usces'); ?>") ) {
			return;
		}
	...
	$(document).on("click", "#reauth-settlement", function() {
		...
		if( !confirm("<?php _e('再オーソリ処理を実行します。よろしいですか？','usces'); ?>") ) {
			return;
		}
	...
	$(document).on("click", "#change-settlement", function() {
		...
		if( paylimit.length != 8 || !checkNum(paylimit) ) {
			alert("<?php _e('支払期限の形式が不正です。8桁の数字で入力してください。','usces'); ?>");
			return;
		}
		if( today > paylimit ) {
			alert("<?php _e('支払期限が不正です。本日以前の日付は指定できません。','usces'); ?>");
			return;
		}
		if( amount == "" || parseInt(amount) === 0 || !checkNum(amount) ) {
			alert("<?php _e('支払額の形式が不正です。数値で入力してください。','usces'); ?>");
			return;
		}
		if( !confirm("<?php _e('支払期限・支払額の変更処理を実行します。よろしいですか？','usces'); ?>") ) {
			return;
		}
	...
	$(document).on("click", "#add-settlement", function() {
		...
		if( !confirm("<?php _e('登録処理を実行します。よろしいですか？','usces'); ?>") ) {
			return;
		}
	...
	$(document).on("click", "#continuation-update", function() {
		...
		if( !confirm("<?php _e('更新します。よろしいですか？','usces'); ?>") ) {
			return;
		}
		...
		case 'get_welcartpay_card':
			...
			if( $trans_id == '9999999999' ) {
				...
				if( 'OK' == $response_member['ResponseCd'] ) {
					$order_data = $usces->get_order_data( $order_id, 'direct' );
					$total_full_price = $order_data['order_item_total_price'] - $order_data['order_usedpoint'] + $order_data['order_discount'] + $order_data['order_shipping_charge'] + $order_data['order_cod_fee'] + $order_data['order_tax'];
					$res .= '<div class="welcart-settlement-admin card-new">'.__('新規','usces').'</div>';
					$res .= '<table class="welcart-settlement-admin-table">';
					$res .= '<tr><th>'.__('利用金額','usces').'</th>
					...
					$res .= '<input id="auth-settlement" type="button" class="button" value="'.__('与信','usces').'" />';
					$res .= '<input id="gathering-settlement" type="button" class="button" value="'.__('与信売上計上','usces').'" />';
					$res .= '</div>';
				} else {
					$res .= '<div class="welcart-settlement-admin-error">';
					$res .= '<div><span class="message">'.__('カード情報未登録','usces').'</span></div>';
					$res .= '</div>';
				}
				die("OK#usces#".$res);
			} else {
				...
				if( 'OK' == $response_data['ResponseCd'] ) {
					...
					if( isset($latest_log['OperateId']) ) {
						...
						if( isset($response_data['SalesDate']) ) {
							$res .= '<tr><th>'.__('売上計上日','usces').'</th><td>'.$response_data['SalesDate'].'</td></tr>';
						}
						$res .= '</table>';
						$res .= '<div class="welcart-settlement-admin-button">';
						if( '1Delete' == $latest_log['OperateId'] ) {
							$res .= '<input id="reauth-settlement" type="button" class="button" value="'.__('再オーソリ','usces').'" />';
						} else {
							if( '1Auth' == $operateid && '1Capture' != $latest_log['OperateId'] ) {
								$res .= '<input id="reauth-settlement" type="button" class="button" value="'.__('再オーソリ','usces').'" />';
								$res .= '<input id="capture-settlement" type="button" class="button" value="'.__('売上計上','usces').'" />';
							}
							if( '1Delete' != $latest_log['OperateId'] ) {
								$res .= '<input id="delete-settlement" type="button" class="button" value="'.__('取消','usces').'" />';
							}
							if( '1Change' != $latest_log['OperateId'] ) {
								$res .= '<input id="change-settlement" type="button" class="button" value="'.__('利用額変更','usces').'" />';
							}
						}
						$res .= '</div>';
					}
		...
		case 'continuation_update':
			...
			if( $continue_data['status'] == 'continuation' && $status == 'cancellation' ) {
				...
				if( !empty($contracted_year) || !empty($contracted_month) || !empty($contracted_day) ) {
					$contracted_date = $contracted_year.'-'.$contracted_month.'-'.$contracted_day;
					if( !$this->isdate($contracted_date) || $contracted_date <= $today ) {
						die("NG#usces#".__('次回契約更新日の値が不正です。','dlseller'));
					}
				} else {
					$contracted_date = '';
				}
				$charged_date = $charged_year.'-'.$charged_month.'-'.$charged_day;
				if( !$this->isdate($charged_date) || $charged_date <= $today ) {
					die("NG#usces#".__('次回引落し日の値が不正です。','dlseller'));
				}
				$continue_data['contractedday'] = $contracted_date;
				$continue_data['chargedday'] = $charged_date;
				$continue_data['price'] = $price;
...
//usces_log(print_r($response_data,true),"test.log");
			if( 'OK' == $response_data['ResponseCd'] ) {
				...
				$res .= '<div class="welcart-settlement-admin conv-noreceipt">'.__('未入金','usces').'</div>';
				$res .= '<table class="welcart-settlement-admin-table">';
				$res .= '<tr><th>'.__('支払期限','usces').'</th>
				...
				$res .= '<tr><th>'.__('支払額','usces').'</th>
					<td><input type="text" id="amount_change" value="'.$amount.'" style="text-align:right;ime-mode:disabled" size="10" />'.__(usces_crcode('return'),'usces').'<input type="hidden" id="amount" value="'.$amount.'" /></td>
					</tr>';
				$res .= '</table>';
				$res .= '<div class="welcart-settlement-admin-button">';
				$res .= '<input id="delete-settlement" type="button" class="button" value="'.__('取消','usces').'" />';
				$res .= '<input id="change-settlement" type="button" class="button" value="'.__('変更','usces').'" />';
				$res .= '</div>';
				$status = '<span class="acting-status conv-noreceipt">'.__('未入金','usces').'</span>';
			} else {
				$res .= '<div class="welcart-settlement-admin-error">';
				$responsecd = explode( '|', $response_data['ResponseCd'] );
				foreach( (array)$responsecd as $cd ) {
...
//usces_log(print_r($response_data,true),"test.log");
			if( 'OK' == $response_data['ResponseCd'] ) {
				...
				if( isset($acting_data['PayLimit']) ) {
					$res .= '<tr><th>'.__('支払期限','usces').'</th><td>'.$acting_data['PayLimit'].'</td></tr>';
				}
				if( isset($acting_data['Amount']) ) {
					$res .= '<tr><th>'.__('支払額','usces').'</th><td>'.$acting_data['Amount'].'</td></tr>';
				}
				$res .= '</table>';
				$res .= '<div class="welcart-settlement-admin-button">';
				$res .= '<input id="delete-settlement" type="button" class="button" value="'.__('取消','usces').'" />';
...
//usces_log(print_r($response_data,true),"test.log");
			if( 'OK' == $response_data['ResponseCd'] ) {
				$res .= '<div class="welcart-settlement-admin conv-del">'.__('取消済み','usces').'</div>';
				$res .= '<table class="welcart-settlement-admin-table">';
				if( isset($acting_data['PayLimit']) ) {
					$paylimit = substr($acting_data['PayLimit'],0,8);
					$res .= '<tr><th>'.__('支払期限','usces').'</th><td>'.$paylimit.'</td></tr>';
					//$res .= '<tr><th>'.__('支払期限','usces').'</th>
					//	<td><input type="text" id="paylimit_change" value="'.$paylimit.'" style="ime-mode:disabled" size="10" /><input type="hidden" id="paylimit" value="'.$paylimit.'" /></td>
					//	</tr>';
				}
				...
				$res .= '</table>';
				//$res .= '<div class="welcart-settlement-admin-button">';
				//$res .= '<input id="add-settlement" type="button" class="button" value="'.__('登録','usces').'" />';
				//$res .= '</div>';
				$status = '<span class="acting-status conv-del">'.__('取消済み','usces').'</span>';
			} else {
				$res .= '<div class="welcart-settlement-admin-error">';
				$responsecd = explode( '|', $response_data['ResponseCd'] );
				foreach( (array)$responsecd as $cd ) {
...
//usces_log(print_r($response_data,true),"test.log");
			if( 'OK' == $response_data['ResponseCd'] ) {
				if( isset($response_data['NyukinDate']) ) {
					$res .= '<div class="welcart-settlement-admin conv-receipted">'.__('入金済','usces').'</div>';
					$res .= '<table class="welcart-settlement-admin-table">';
					if( isset($response_data['RecvNum']) ) {
						$res .= '<tr><th>'.__('受付番号','usces').'</th><td>'.$response_data['RecvNum'].'</td></tr>';
					}
					if( isset($response_data['NyukinDate']) ) {
						$res .= '<tr><th>'.__('入金日時','usces').'</th><td>'.$response_data['NyukinDate'].'</td></tr>';
					}
					if( isset($response_data['CvsCd']) ) {
						$cvs_name = $this->get_cvs_name($response_data['CvsCd']);
						$res .= '<tr><th>'.__('収納機関コード','usces').'</th><td>'.$cvs_name.'</td></tr>';
					}
					if( isset($response_data['TenantCd']) ) {
						$res .= '<tr><th>'.__('店舗コード','usces').'</th><td>'.$response_data['TenantCd'].'</td></tr>';
					}
					if( isset($response_data['Amount']) ) {
						$res .= '<tr><th>'.__('支払額','usces').'</th><td>'.$response_data['Amount'].__(usces_crcode('return'),'usces').'</td></tr>';
					}
					$res .= '</table>';
				} else {
					$paylimit = substr($acting_data['PayLimit'],0,8);
					$expiration = $this->check_paylimit( $order_id, $trans_id );
					$res .= '<div class="welcart-settlement-admin conv-noreceipt">'.__('未入金','usces');
					if( $expiration ) {
						$res .= __('（期限切れ）','usces');
					}
					$res .= '</div>';
		...
		if( $log_data ) {
			...
			$html = '<table class="settlement-history">
				<thead class="settlement-history-head">
					<tr><th></th><th>'.__('処理日時','usces').'</th><th>'.__('処理通番','usces').'</th><th>'.__('処理区分','usces').'</th><th>'.__('処理結果','usces').'</th></tr>
				</thead>
				<tbody class="settlement-history-body">';
			foreach( (array)$log_data as $data ) {
				$log = unserialize( $data['log'] );
				$class = ( $log['ResponseCd'] != 'OK' ) ? ' class="error"' : '';
				...
				$html .= '<tr'.$class.'>
					...
					<td class="responsecd">'.$log['ResponseCd'].'</td>
				</tr>';
				$num--;
			}
			$html .= '</tbody>
	...
	* @return -
	***********************************************/
	public function output_atodine_csv( $order_action ) {

		switch( $order_action ) {
		case 'download_atodine_register':
			$this->download_atodine_register();
			break;
		case 'download_atodine_update':
			$this->download_atodine_update();
			break;
		case 'download_atodine_report':
			$this->download_atodine_report();
			break;
		case 'upload_atodine_results':
			if( isset($_GET['atodine_upfile']) && !WCUtils::is_blank($_GET['atodine_upfile']) ) {
				$this->upload_atodine_results();
			}
			break;
		}
	}
	...
	* @return -
	***********************************************/
	public function download_atodine_register() {
		global $usces;

		$jptime = time()+(3600 * 9);
		$filename = mb_convert_encoding("取引登録", 'SJIS', 'UTF-8').gmdate("YmdHis", $jptime).".csv";
		$line = "";
		$ids = $_GET['listcheck'];

		$line_header = "ご購入店受注番号,購入者注文日,会社名,部署名,氏名,氏名（フリガナ）,郵便番号,住所,電話番号,メールアドレス,配送先会社名,配送先部署名,配送先氏名,配送先氏名（フリガナ）,配送先住所,配送先電話番号,請求書送付方法,予備領域1,予備領域2,予備領域3,顧客請求金額（税込）,明細名（商品名）,単価（税込）,数量\r\n";

		foreach( (array)$ids as $order_id ) {
			$order_data = $usces->get_order_data( $order_id, 'direct' );
			$payment = $usces->getPayments( $order_data['order_payment_name'] );
			if( 'acting_welcart_atodine' != $payment['settlement'] ) {
				continue;
			}
			...
			$order_zip = str_replace('ー', '-', mb_convert_kana($order_data['order_zip'], 'a'));
			$order_post = str_replace("-", "", $order_zip);
			$order_address = $order_data['order_pref'].$order_data['order_address1'];
			if( !empty($order_data['order_address2']) ) $orderer_address .= mb_convert_kana($order_data['order_address2'], 'ak', 'UTF-8');
			if( !empty($order_data['order_address3']) ) $orderer_address .= mb_convert_kana($order_data['order_address3'], 'ak', 'UTF-8');
			...
			if( 0 < $cart_cnt ) {
				...
				$itemunit = $usces->getItemSkuUnit( $post_id, $sku );
				if( 1 < $cart_cnt ) {
					$cart_cnt--;
					$itemname .= "　その他".mb_convert_kana($cart_cnt, 'N', 'UTF-8')."件";
				}
			}
			...
			$line .= $order_number.','.
				...
				$itemname.'",,"'.
				$itemunit.'",,,"'.
				$amount.','.",0,,,2,123456789012,,11,0,,,,,0,,,,,,,0,0,,,,,,,,,,,,,,,,,\r\n";
		}
	...
	* @return -
	***********************************************/
	public function download_atodine_update() {
		global $usces;
	}
	...
	* @return -
	***********************************************/
	public function download_atodine_report() {
		global $usces;
	}
	...
	* @return -
	***********************************************/
	public function upload_atodine_results() {
		global $usces;

		$path = WP_CONTENT_DIR.'/uploads/';

		$file_name = $_GET['atodine_upfile'];
		$decode_filename = base64_decode( $file_name );
		if( !file_exists( $path.$file_name ) ) {
			echo 'error : '.__('CSV file does not exist.','usces').esc_html( $decode_filename );
			return;
		}
		...
		if( !( $fpo = fopen( $path.$file_name, "r" ) ) ) {
			echo 'error : '.__('A file does not open.','usces').esc_html( $decode_filename );
			return;
		}
		...
		if( 'csv' !== end( $fname_parts ) ) {
			echo 'error : '.__('This file is not in the CSV file.','usces').esc_html( $decode_filename );
			return;
		}
		...
		foreach( $orglines as $line ) {
			list( $order_id, $atodine_number, $name, $amount, $billing_method, $results ) = explode( $sp, $line );
			if( 'OK' == $results ) {
				usces_change_order_receipt( (int)$order_id, 'receipted' );
			}
			$usces->set_order_meta_value( 'atodine_number', $atodine_number, (int)$order_id );
			$data = array( $amount, $billing_method, $results );
			$usces->set_order_meta_value( 'atodine_results', serialize($data), (int)$order_id );
		}
	...
	* @return -
	***********************************************/
	public function action_atodine_button() {
?>
				<input type="button" id="dl_atodine_register_csv" class="searchbutton" value="後払い決済取引登録CSV出力" />
				<input type="button" id="dl_atodine_update_csv" class="searchbutton" value="後払い決済取引一括変更・キャンセルCSV出力" />
				<input type="button" id="up_atodine_results_csv" class="searchbutton" value="後払い決済与信審査結果CSV取込" />
				<input type="button" id="dl_atodine_report_csv" class="searchbutton" value="後払い決済出荷報告登録CSV出力" />
<?php
	}
	...
	* @return -
	***********************************************/
	function atodine_upload() {
		global $usces;

		if( isset($_REQUEST['page']) && $_REQUEST['page'] == 'atodine_results_csv' && isset($_REQUEST['action']) && $_REQUEST['action'] == 'atodine_upload' ) {
			$path = WP_CONTENT_DIR.'/uploads/';
			$workfile = $_FILES["atodine_upcsv"]["tmp_name"];
			if( !is_uploaded_file( $workfile ) ) {
				$message = __('The file was not uploaded.','usces');
				wp_redirect( add_query_arg( array( 'page'=>'usces_orderlist', 'atodine_status'=>'error', 'atodine_message'=>urlencode( $message ) ), USCES_ADMIN_URL ) );
				exit();
			}

			list( $fname, $fext ) = explode( '.', $_FILES["atodine_upcsv"]["name"], 2 );
			if( $fext != 'csv' ) {
			...
			if( !move_uploaded_file( $_FILES['usces_upcsv']['tmp_name'], $path.$new_filename ) ) {
				$message = __('The file was not stored.','usces').$_FILES["atodine_upcsv"]["name"];
				wp_redirect( add_query_arg( array( 'page'=>'usces_orderlist', 'atodine_status'=>'error', 'atodine_message'=>urlencode( $message ) ), USCES_ADMIN_URL ) );
				exit();
			}

			wp_redirect( add_query_arg( array( 'page'=>'usces_orderlist', 'order_action'=>'upload_atodine_results', 'atodine_upfile'=>$new_filename ), USCES_ADMIN_URL ) );
			exit();
		}
	}
	...
	***********************************************/
	public function order_list_footer() {
		...
		$html = '
		<div id="atodine_upload_dialog">
			<p>規定のCSVファイルをアップロードして与信審査結果の取込を行います。<br />ファイルを選択して取込開始を押してください。</p>
			<form action="'.USCES_ADMIN_URL.'" method="post" enctype="multipart/form-data" name="atodine_upform" id="atodine_upform">
				<p><input name="atodine_upcsv" type="file" class="filename" /></p>
				<p><input name="atodine_uploadcsv" type="submit" class="button" value="取込開始" /></p>
				<input name="page" type="hidden" value="atodine_results_csv" />
				<input name="action" type="hidden" value="atodine_upload" />
			</form>
		</div>';
		echo $html;
	}
	...
	***********************************************/
	public function order_list_page_js() {
		...
		$html = '
		$(document).on( "click", "#dl_atodine_register_csv", function() {
			...
			location.href = "'.USCES_ADMIN_URL.'?page=usces_orderlist&order_action=download_atodine_register"+listcheck+"&noheader=true&nonce='.wp_create_nonce('csv_nonce').'";
		});

		$(document).on( "click", "#dl_atodine_update_csv", function() {
			if( $("input[name*=\'listcheck\']:checked").length == 0 ) {
				alert("'.__('Choose the data.','usces').'");
				$("#orderlistaction").val("");
				return false;
			...
			location.href = "'.USCES_ADMIN_URL.'?page=usces_orderlist&order_action=download_atodine_update"+listcheck+"&noheader=true&nonce='.wp_create_nonce('csv_nonce').'";
		});

		$(document).on( "click", "#up_atodine_results_csv", function() {
			$("#atodine_upload_dialog").dialog( "open" );
		});

		$(document).on( "click", "#dl_atodine_report_csv", function() {
			if( $("input[name*=\'listcheck\']:checked").length == 0 ) {
				alert("'.__('Choose the data.','usces').'");
				$("#orderlistaction").val("");
				return false;
			...
			location.href = "'.USCES_ADMIN_URL.'?page=usces_orderlist&order_action=download_atodine_report"+listcheck+"&noheader=true&nonce='.wp_create_nonce('csv_nonce').'";
		});

		$("#atodine_upload_dialog").dialog({
			bgiframe: true,
			autoOpen: false,
			title: "'.__('与信審査結果CSV取込','usces').'",
			height: 300,
			width: 500,
			modal: true,
			buttons: {
	...
	***********************************************/
	public function order_list_action_status( $status ) {
		if( isset($_GET['atodine_status']) ) {
			$status = $_GET['atodine_status'];
		}
		return $status;
	}
	public function order_list_action_message( $message ) {
		if( isset($_GET['atodine_message']) ) {
			$message = $_GET['atodine_message'];
		}
		return $message;
	}
	...
	public function set_fee_label( $label ) {
		...
		if( is_admin() ) {
			...
			if( !empty($order_id) ) {
				$order_data = $usces->get_order_data( $order_id, 'direct' );
				$payment = usces_get_payments_by_name( $order_data['order_payment_name'] );
				if( 'acting_welcart_conv' == $payment['settlement'] || 'acting_welcart_atodine' == $payment['settlement'] ) {
	...
	***********************************************/
	public function payment_method( $payments ) {
		...
		$atodine_exclusion = false;

		if( usces_have_regular_order() || usces_have_continue_charge() ) {
			$conv_exclusion = true;
			$atodine_exclusion = true;

		} else {
			$acting_opts = $this->get_acting_settings();
			if( 'on' == $acting_opts['atodine_byitem'] ) {
				$cart = $usces->cart->get_cart();
				foreach( $cart as $cart_row ) {
					$atodine_propriety = get_post_meta( $cart_row['post_id'], 'atodine_propriety', true );
					if( 1 == (int)$atodine_propriety ) {
						$atodine_exclusion = true;
						break;
					}
				}
	...
	***********************************************/
	public function check_fee_limit( $mes ) {
		...
		if( 2 == $usces_entries['delivery']['delivery_flag'] ) {
			$mes .= '複数配送先をご指定の場合は、「'.$usces_entries['order']['payment_name'].'」のお支払い方法はご利用できません。';
			return $mes;
		}
		...
		case 'acting_welcart_atodine':
			if( 'fix' == $acting_opts['atodine_fee_type'] ) {
				$fee_limit_amount = $acting_opts['atodine_fee_first_amount'];
			}
			break;
		}

		if( 0 < $fee_limit_amount && $usces_entries['order']['total_full_price'] > $fee_limit_amount ) {
			$mes .= '「'.$usces_entries['order']['payment_name'].'」の上限額（総合計金額 '.usces_crform( $fee_limit_amount, true, false, 'return', true ).'）を超えています。';
		}
	...
	***********************************************/
	public function edit_item_atodine_byitem( $second_section, $post_id ) {
		global $usces;

		$division = $usces->getItemDivision( $post_id );
		$charging_type = $usces->getItemChargingType( $post_id );
		$acting_opts = $this->get_acting_settings();
		if( 'shipped' == $division && 'once' == $charging_type && 'on' == $acting_opts['atodine_byitem'] ) {
			$atodine_propriety = get_post_meta( $post_id, 'atodine_propriety', true );
			$checked = ( 1 == (int)$atodine_propriety ) ? array( '', ' checked="checked"' ) : array( ' checked="checked"', '' );
			$second_section .= '
			<tr>
				<th>'.__('Atobarai Propriety','usces').'</th>
				<td>
					<label for="atodine_propriety_0"><input name="atodine_propriety" id="atodine_propriety_0" type="radio" value="0"'.$checked[0].'>'.__('available','usces').'</label>
					<label for="atodine_propriety_1"><input name="atodine_propriety" id="atodine_propriety_1" type="radio" value="1"'.$checked[1].'>'.__('not available','usces').'</label>
				</td>
			</tr>';
		}
	...
	* @return -
	***********************************************/
	public function save_item_atodine_byitem( $post_id, $post ) {

		if( isset($_POST['atodine_propriety']) ) {
			update_post_meta( $post_id, 'atodine_propriety', $_POST['atodine_propriety'] );
		}
	}
	...
	***********************************************/
	public function nonacting_settlements( $nonacting_settlements ) {
		...
		if( !in_array( 'acting_welcart_atodine', $nonacting_settlements ) ) {
			$nonacting_settlements[] = 'acting_welcart_atodine';
		}
	...
	* @return str $message
	***********************************************/
	private function response_message( $code ) {
	...
	***********************************************/
	private function error_message( $code ) {
		...
		case 'W99'://その他例外エラー
			$message = '恐れ入りますが、お問い合わせフォームより管理者にお問い合わせください。';
			break;
		...
		case 'G42'://暗証番号エラー
		case 'G84'://承認番号エラー
			$message = 'カード情報が適切ではありません。';
			break;
		case 'C15'://ボーナス金額下限エラー
			$message = 'ボーナス払いの下限金額未満によるエラーのため、支払方法を変更して再処理を行ってください。';
			break;
		case 'G12'://カード使用不可
		...
		case 'G97'://当該要求拒否
			$message = 'クレジットカードが使用不可能です。 ';
			break;
		case 'G54'://利用回数エラー
			$message = '1日利用回数または金額オーバーです。';
			break;
		case 'G55'://限度額オーバー
			$message = '1日利用限度額オーバーです。';
			break;
		case 'G68'://金額エラー
		case 'G72'://ボーナス額エラー
			$message = '金額が適切ではありません。';
			break;
		case 'G75'://分割金額エラー
			$message = '分割払いの下限金額を下回っています。';
			break;
		case 'K28':
			$message = 'お客様電話番号が適切ではありません。';
			break;
		case 'K51'://項目「NameKanji」精査エラー
			$message = 'お客様氏名が適切に入力されていません。';
			break;
		case 'K52'://項目「NameKana」精査エラー
			$message = 'お客様氏名カナが適切に入力されていません。';
			break;
		default:
			$message = '恐れ入りますが、お問い合わせフォームより管理者にお問い合わせください。';
		}
	...
	class SLNConnection
	{
		...
		function send_request( &$param_list = array() )
		{
			...
			if( empty($param_list) === false ) {
				...
				if( $fp === false ) {
					usces_log('[WelcartPay] e-SCOTT send error : '."TLS1.2接続に失敗しました", 'acting_transaction.log');
					$fp = @stream_socket_client( 'ssl://'.$url['host'].':443', $errno, $errstr, $this->connection_timeout, STREAM_CLIENT_CONNECT, $context );
					if( $fp === false ) {
						usces_log('[WelcartPay] e-SCOTT send error : '."SSL接続に失敗しました", 'acting_transaction.log');
						return $rValue;
					}
				}
					...
					if( $result === true ) {
						...
						if( $result === true ) {
							if( $hm['timed_out'] !== true ) {
								...
								// エラー： タイムアウト発生
								usces_log('[WelcartPay] e-SCOTT send error : '."通信中にタイムアウトが発生しました", 'acting_transaction.log');
							}
						} else {
							// エラー： ソケット通信切断失敗
							usces_log('[WelcartPay] e-SCOTT send error : '."SLNとの切断に失敗しました", 'acting_transaction.log');
						}
					} else {
						// エラー： タイムアウト設定失敗 
						usces_log('[WelcartPay] e-SCOTT send error : '."タイムアウト設定に失敗しました", 'acting_transaction.log');
					}
				}
			} else {
				// エラー： パラメータ不整合
				usces_log('[WelcartPay] e-SCOTT send error : '."リクエストパラメータの指定が正しくありません", 'acting_transaction.log');
			}
			return $rValue;
		}
	}
