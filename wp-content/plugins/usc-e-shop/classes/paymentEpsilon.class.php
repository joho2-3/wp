...
class EPSILON_SETTLEMENT
{
	...
	***********************************************/
	private function set_available_settlement() {
		...
		$available_settlement['epsilon'] = 'イプシロン';
...
//usces_log("send_data=".print_r($send_data,true),"epsilon.log");
		...
		//	],
		//]);
		//$fp = stream_socket_client( 'tls://'.$interface['host'].':443', $errno, $errstr, 30, STREAM_CLIENT_CONNECT, $ctx );
		...
		//usces_log("meta=".print_r($meta,true),"test.log");
	...
	<form action="" method="post" name="epsilon_form" id="epsilon_form">
		<table class="settle_table">
			<tr>
				...
				<td colspan="6"><input name="contract_code" type="text" id="contract_code_epsilon" value="<?php echo (isset($opts['epsilon']['contract_code']) ? $opts['epsilon']['contract_code'] : ''); ?>" size="20" maxlength="8" /></td>
		...
		<input name="usces_option_update" type="submit" class="button button-primary" value="イプシロンの設定を更新する" />
	...
	***********************************************/
	public function register_order_data( $args ) {
		...
		if( isset($_REQUEST['acting']) && isset($_REQUEST['acting_return']) && isset($_REQUEST['trans_code']) && 'epsilon' == $_REQUEST['acting'] ) {
			if( !empty($member_id) ) {
				$release = $usces->get_member_meta_value( 'epsilon_process_code_release', $member_id );
				if( $release ) {
					$usces->del_member_meta( 'epsilon_process_code_release', $member_id );
				}
			}
