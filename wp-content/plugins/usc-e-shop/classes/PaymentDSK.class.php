...
class DSK_SETTLEMENT
{
	...
	public function __construct(){
		...
		if( is_admin() ){
			...
			if( $this->is_activate( 'acting_dsk_card' ) || $this->is_activate( 'acting_dsk_conv' ) || $this->is_activate( 'acting_dsk_payeasy' )){
			...
			if( $this->is_activate( 'acting_dsk_card' ) ){
				add_filter( 'usces_filter_is_complete_settlement', array( $this, 'is_complete_settlement'), 10, 3 );
			}

			if( $this->is_activate( 'acting_dsk_conv' ) || $this->is_activate( 'acting_dsk_payeasy' )){
	...
	***********************************************/
	private function set_available_settlement() {
		...
		if( !in_array( 'dsk', $available_settlement ) ) {
			$available_settlement['dsk'] = 'DSKペイメント';
			update_option( 'usces_available_settlement', $available_settlement );
		}
	}

	public function is_activate( $settlement ){
		$res = false;
		...
		if(!isset($options['acting_settings']['dsk']))
			return $res;
		
		switch( $settlement ){
			case 'acting_dsk_card':
				if( 'on' == $options['acting_settings']['dsk']['card_activate'] && (isset($payments[$settlement]) && 'activate' == $payments[$settlement]['use']) ){
					$res = true;
				}
				break;
			case 'acting_dsk_conv':
				if( 'on' == $options['acting_settings']['dsk']['conv_activate'] && (isset($payments[$settlement]) && 'activate' == $payments[$settlement]['use']) ){
					$res = true;
				}
				break;
			case 'acting_dsk_payeasy':
				if( 'on' == $options['acting_settings']['dsk']['payeasy_activate'] && (isset($payments[$settlement]) && 'activate' == $payments[$settlement]['use']) ){
					$res = true;
				}
				break;
		}
	...
	***********************************************/
	public function check_acting_return_duplicate( $tid, $results ){
		if( !isset($_REQUEST['free1']) || !isset($_REQUEST['order_id']) )
			return $tid;
	
		$acting = $_REQUEST['free1'];
...
-->			<?php endif; ?>
		...
		<input name="usces_option_update" type="submit" class="button button-primary" value="DSKペイメントの設定を更新する" />
