...
//		}
	...
	//		usces_p($cartItemName);
			if( 100 < strlen($cartItemName) ){
				$cartItemName = mb_substr($cartItemName, 0, 40) . '･･･';
			}
	//		usces_p($cartItemName);
	//		die();
...
//			usces_log('yahoo_responce : '.print_r($simplexml, true), 'yahoo_error.log', 'test');
			global $usces, $wpdb;
	...
	<form action="" method="post" name="yahoo_form" id="yahoo_form">
		<table class="settle_table">
			...
			<tr>
				...
				<td colspan="6"><input name="merchant_key" type="text" id="merchant_key_yahoo" value="<?php echo esc_html(isset($opts['yahoo']['merchant_key']) ? $opts['yahoo']['merchant_key'] : ''); ?>" size="50" /></td>
			...
			<tr>
				<th><a style="cursor:pointer;" onclick="toggleVisibility('ex_expire_yahoo');">支払期限時間</a></th>
				<td colspan="6"><input name="expire_yahoo" type="text" id="expire_yahoo" value="<?php echo esc_html(isset($opts['yahoo']['expire']) ? $opts['yahoo']['expire'] : '10'); ?>" size="10" />時間</td>
				<td><div id="ex_expire_yahoo" class="explanation"><?php _e('支払URLの有効時間。初期値は10時間です。', 'usces'); ?></div></td>
			</tr>
		</table>
