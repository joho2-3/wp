...
<table>
	...
	<tr>
		...
		<?php if($order_action == 'new') : ?>
		<td colspan="2" class="midasi1"><?php _e('shipping address', 'usces'); ?><input type="button" class="button" id="costomercopy" value="<?php _e("Same shipping address", "usces"); ?>"></td>
		<?php else : ?>
			...
			<table border="0" cellspacing="0" class="cus_info">
				...
				<tr>
					...
					<td class="col1">
						...
						<?php
						$delivery_days_select = '<option value="'.$no_preference.'">'.$no_preference.'</option>';
						...
						for($i = 0; $i < $delivery_after_days; $i++) {
							...
							$date = date(__( 'M j, Y', 'usces_dual' ), mktime(0,0,0,$order_date[1],$order_date[2]+$i,$order_date[0]));
				...
				<tr>
					...
					<td class="col1">
						<select name="offer[delidue_date]">
						    <option value="#none#"><?php _e('Not notified', 'usces_dual'); ?></option>
						<?php
						//for ($i=0; $i<50; $i++) {
						for($i = 0; $i < $delivery_after_days; $i++) {
							$value = date('Y-m-d', mktime(0,0,0,date('m'),date('d')+$i,date('Y')));
							$date = date(__( 'M j, Y', 'usces_dual' ), mktime(0,0,0,date('m'),date('d')+$i,date('Y')));
...
<div class="ordernavi"><input name="upButton2" class="button button-primary" type="submit" value="<?php _e('change decision', 'usces'); ?>" /><?php _e("When you change amount, please click 'Edit' before you finish your process.", 'usces'); ?></div>
</div>
