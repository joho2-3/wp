...
Modified Date:10 May 2015
Modified by: Communitycom.Inc
*/
class saitama_bread_crumb{

public function bread_crumb( $args = '' ) {

	$default = array(
		'type'				=> 'list',
		'home_label'		=> __( 'Home', 'saitama' ),
		'search_label'		=> __( 'Search Result of "%s"', 'saitama' ),
		'404_label'			=> __( '404 Not Found', 'saitama' ),
		'category_label'	=> _x( '%s', 'category label', 'saitama' ),
		'tag_label'			=> _x( '%s', 'tag label', 'saitama' ),
		'taxonomy_label'	=> _x( '%s', 'taxonomy label', 'saitama' ),
		'author_label'		=> _x( '%s', 'author label', 'saitama' ),
		'attachment_label'	=> _x( '%s', 'attachment label', 'saitama' ),
		'year_label'		=> _x( '%s', 'year label', 'saitama' ),
		'month_label'		=> _x( '%s', 'month label', 'saitama' ),
		'day_label'			=> _x( '%s', 'day label', 'saitama' ),
		'post_type_label'	=> _x( '%s', 'post type label', 'saitama' ),
		'joint_string'		=> __( ' &gt; ', 'saitama' ),
		'navi_element'		=> '',
...
private function get_bread_crumb_array( $args ) {
	...
	$bread_crumb_arr[] = array( 'title' => $args['home_label'], 'link' => home_url('/') );
	...
	if ( is_404() ) {
		...
		$bread_crumb_arr[] = array( 'title' => sprintf( $args['author_label'], get_the_author_meta('display_name') ), 'link' => get_author_posts_url( $author ) );
	} elseif ( is_day() ) {
...
$saitama_bread_crumb = new saitama_bread_crumb();

if ( ! function_exists( 'bread_crumb' ) ) {
function bread_crumb( $args = '' ) {
	global $saitama_bread_crumb;
	return $saitama_bread_crumb->bread_crumb( $args );
}
}